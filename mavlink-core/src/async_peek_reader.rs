...
impl<R: AsyncRead + Unpin, const BUFFER_SIZE: usize> AsyncPeekReader<R, BUFFER_SIZE> {
    /// Instantiates a new [`AsyncPeekReader`], wrapping the provided [`tokio::io::AsyncRead`] and using the default chunk size
    ...
    /// from the underlying [`tokio::io::AsyncRead`] until it does, an error occurs or no more data can be read (EOF).
    ///
